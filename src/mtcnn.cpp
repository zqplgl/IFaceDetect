...
FaceDetector::FaceDetector(const string& model_dir, const MODEL_VERSION model_version)
{
    ...
    /* load three networks */
    //p
    P_Net.reset( new Net<float> (model_dir + "mtcnn/det1.prototxt", TEST) );
    P_Net->CopyTrainedLayersFrom(model_dir + "mtcnn/det1.caffemodel");
    //R
    R_Net.reset( new Net<float> (model_dir + "mtcnn/det2.prototxt", TEST) );
    R_Net->CopyTrainedLayersFrom(model_dir + "mtcnn/det2.caffemodel");
    //O
    O_Net.reset( new Net<float> (model_dir + "mtcnn/det3.prototxt", TEST) );
    O_Net->CopyTrainedLayersFrom(model_dir + "mtcnn/det3.caffemodel");
    //L
    if( model_version == MODEL_V2)
    {
        L_Net.reset( new Net<float> (model_dir + "mtcnn/det4.prototxt", TEST) );
        L_Net->CopyTrainedLayersFrom(model_dir + "mtcnn/det4.caffemodel");
    }
...
void FaceDetector::detection(const cv::Mat& img, std::vector<float>& confidence, std::vector<BoundingBox>& alignment)
{
    alignment.clear();
    vector<BoundingBox> res = Detect(img, FaceDetector::BGR, FaceDetector::ORIENT_UP ,60, confidence[0], confidence[1], confidence[2]);

    for(int i=0; i<res.size(); ++i)
    {
        alignment.push_back(res[i]);
    }
}
...
void FaceDetector::filteroutBoundingBox(const vector< BoundingBox >& boxes, 
 ...
 */
vector<BoundingBox> FaceDetector::Detect(const cv::Mat& img, const COLOR_ORDER color_order, const IMAGE_DIRECTION orient, int min_size, float P_thres, float R_thres, float O_thres, bool is_fast_resize, float scale_factor)
{
    /*change image format*/
    cv::Mat sample;
...
IObjZoneDetect *CreateObjZoneMTcnnDetector(const string &model_dir)
{
    ...
    IObjZoneDetect *detector = new FaceDetector(temp, FaceDetector::MODEL_V1);
    return detector;

}

//int main(int argc, char **argv) {
//    FaceDetector fd("/home/zqp/install_lib/models/mtcnn", FaceDetector::MODEL_V1);
//    
//    vector<string> imgList;
//    readFileList("/home/zqp/pic/face", imgList);
